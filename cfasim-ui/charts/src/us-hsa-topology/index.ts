 ...
 *
 * Contains `objects.hsas` (one merged geometry per mapped HSA, ids are 6-char
 * HSA codes: state FIPS + 4-digit HSA number) and an unchanged
 * `objects.states`.
 ...
 * cannot project) are omitted, matching what the runtime merge renders.
 * County-level maps and `geoType: "counties"` data rows still require a
 * counties topology.
 */
